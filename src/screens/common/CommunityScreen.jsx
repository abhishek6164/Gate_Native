...
import { getResidents } from "../../services/authApi";
...
export default function CommunityScreen() {
  ...
  useEffect(() => {
    const fetchResidents = async () => {
      try {
        ...
        setResidentsData(data.residents);
      } catch (error) {
  ...
  const getRoleTheme = (role) => {
    switch (role) {
      case "FAMILY": return { bg: "#ecfdf5", text: "#059669", circle: "#10b981" };
      ...
      <TouchableOpacity activeOpacity={0.7} style={styles.card}>
        <View style={styles.leftSection}>
          ...
          <View style={styles.info}>
            ...
            <Text style={styles.flatText}>📍 Bldg {item.building} - {item.flat}</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  searchContainer: {
    ...
    marginHorizontal: 20, marginVertical: 15, paddingHorizontal: 15,
    borderRadius: 16, borderWidth: 1, borderColor: "#e2e8f0",
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.05, shadowRadius: 10, elevation: 2,
  },
  searchIcon: { fontSize: 16, marginRight: 10 },
  searchInput: { flex: 1, height: 50, fontSize: 15, color: "#1e293b" },
  ...
  info: { flex: 1 },
  nameText: { fontSize: 16, fontWeight: "700", color: "#334155" },
  flatText: { fontSize: 13, color: "#64748b", fontWeight: "500", marginTop: 4 },
  rightSection: { alignItems: "flex-end" },
  roleBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, marginBottom: 6 },
  roleText: { fontSize: 11, fontWeight: "800", letterSpacing: 0.5 },
  mobileText: { fontSize: 12, color: "#94a3b8", fontWeight: "500" },
